...
export default function Avatar({image, size = 'large', highlight = false}: Props) {
  ...
  return <div className={getContainerStyle(size, highlight)}>
    ...
    <img className={`bg-white rounded-full ${getImageSizeStyle(size)}`} alt='user profile' src={image ?? undefined} referrerPolicy='no-referrer' />
  </div>
}
...
function getContainerSize(size: string) {
  switch (size) {
